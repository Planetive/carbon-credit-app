...
import type { EndOfLifeRow } from "@/components/emissions/scope3/types/scope3Types";
import type { WasteMaterial, DisposalMethod } from "@/components/emissions/scope3/wasteTypes";
import { getAvailableDisposalMethods } from "@/components/emissions/scope3/wasteTypes";

interface EndOfLifeTreatmentSectionProps {
  rows: EndOfLifeRow[];
  ...
  onAddRow: () => void;
  onUpdateRow: (id: string, patch: Partial<EndOfLifeRow>) => void;
...
export const EndOfLifeTreatmentSection: React.FC<EndOfLifeTreatmentSectionProps> = ({
  rows,
  wasteMaterials,
  deletingIds,
  totalEmissions,
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={onAddRow} className="bg-teal-600 hover:bg-teal-700 text-white">
      ...
      <div className="space-y-4">
        {rows.map((r) => {
          ...
          return (
            <div
              key={r.id}
              className="grid grid-cols-1 md:grid-cols-6 gap-4 items-center p-4 rounded-lg bg-gray-50 border border-gray-200"
            >
              ...
              <div className="w-full">
                <Input
                  ...
                  onChange={(e) => {
                    const value = e.target.value;
                    if (value === "") {
                      onUpdateRow(r.id, { recycle: undefined });
                    } else {
                      const numValue = Number(value);
                      if (numValue >= 0 && numValue <= 100) {
                        onUpdateRow(r.id, { recycle: numValue });
                      }
                    }
                  }}
                  placeholder="Recycle %"
              ...
              <div className="w-full">
                <Input
                  value={r.composition}
                  onChange={(e) => onUpdateRow(r.id, { composition: e.target.value })}
      ...
      <div className="flex items-center justify-between pt-4 border-t">
        <div className="text-gray-700 font-medium">
          ...
          Total Emissions:{" "}
          <span className="font-semibold">{totalEmissions.toFixed(2)} kg CO2e</span>
        </div>
        <div className="flex items-center gap-2">
          <Button
            onClick={onSave}
            disabled={saving || savePendingCount === 0}
            className="bg-teal-600 hover:bg-teal-700 text-white"
          >
            <Save className="h-4 w-4 mr-2" />
            {saving ? "Saving..." : `Save (${savePendingCount})`}
          </Button>
          {onSaveAndNext && (
            <Button
              variant="outline"
