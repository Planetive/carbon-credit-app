...
import type { UpstreamTransportRow } from "@/components/emissions/scope3/types/scope3Types";
import type { VehicleType } from "@/components/emissions/scope3/vehicleTypes";
import { getVehicleTypeNote, getVehicleTypeSuperscript, cleanVehicleTypeName } from "@/components/emissions/scope3/utils/vehicleTypeHelpers";
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@/components/ui/tooltip";

interface UpstreamTransportationSectionProps {
  rows: UpstreamTransportRow[];
  vehicleTypes: VehicleType[];
  deletingIds: Set<string>;
  totalEmissions: number;
  totalWeight: number;
  onAddRow: () => void;
  onUpdateRow: (id: string, patch: Partial<UpstreamTransportRow>) => void;
...
export const UpstreamTransportationSection: React.FC<UpstreamTransportationSectionProps> = ({
  rows,
  vehicleTypes,
  deletingIds,
  totalEmissions,
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600">
            Transport modes, distances, vehicle types, fuel use
          </p>
        </div>
        <Button
          onClick={onAddRow}
            ...
            >
              <div className="w-full flex items-center gap-2">
                <Select
                  ...
                  onValueChange={(v) => onUpdateRow(r.id, { vehicleTypeId: v })}
                >
                    ...
                    <TooltipProvider>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          ...
                          >
                            <Info className="h-4 w-4 text-[#1D9E75] hover:text-[#0F6E56]" />
                          </button>
                        </TooltipTrigger>
