...
export function calcTotalVolumeReleased(rows: SpillRow[]): number {
  return rows.reduce((sum, r) => sum + (r.spillVolumeCubicM || 0), 0);
}
...
export function calcPctVolumeRecovered(rows: SpillRow[]): number {
  const released = calcTotalVolumeReleased(rows);
...
export function formatBioNum(n: number, digits = 2): string {
