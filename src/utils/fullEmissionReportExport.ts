...
export async function exportFullEmissionReportPdf(opts: FullEmissionReportExportOptions): Promise<void> {
  ...
  const results = opts.results;
      ...
      const downstreamScope3Rows = [...results.scope3]
        .filter((r) => ["investments", "downstream_transport", "end_of_life", "processing_sold", "use_of_sold", "facilitated"].includes(r.key) && r.value > 0)
        .sort((a, b) => b.value - a.value);
      ...
      const scopeKeyToTable: Record<string, string> = {
        ...
        refrigerant: "scope1_refrigerant_entries",
        passenger: "scope1_passenger_vehicle_entries",
        delivery: "scope1_delivery_vehicle_entries",
        epa_scope1_heat_steam: "scope1_heatsteam_entries_epa",
      ...
      pdf.save(downloadFileName ?? `Emission_Results_Report_${fileDate}.pdf`);
}
